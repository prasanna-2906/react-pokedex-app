    ...
    <div className="mt-32 px-4 md:px-10 mb-20">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 2xl:grid-cols-5 gap-y-9 gap-x-6 justify-items-center">
        ...
        {pokemonData.map((item) => {
          ...
          const iconPath = typeIcon[primaryType];
          ...
          return (
            ...
            >
              ...
              <p className="flex items-center gap-2 z-10 px-4 py-1.5 font-display bg-white/20  border border-white/30 rounded-full shadow-sm font-extrabold text-lg uppercase tracking-[0.2em] text-white/90  mb-1 drop-shadow-md">
                {iconPath && (
                  <div
                    className="w-6 h-6 bg-white" // This color is what the icon will be
                    style={{
              ...
              <span className="self-center z-10 text-white font-black opacity-10 text-lg">
              ...
              <p className="z-10 text-3xl text-white capitalize font-mono font-black mt-8 drop-shadow-lg">
                {item.name}
              </p>
              <div className="z-10 flex gap-3 mt-5">
                {item.types.map((t) => {
                  const typeName = t.type.name;
                  const iconPath = typeIcon[typeName]; // Look up the specific icon path for this type

                  return (
                    <span
                      key={typeName}
                      className="flex items-center gap-2 px-4 py-1.5 font-display bg-white/20 backdrop-blur-md border border-white/30 rounded-full text-md font-bold text-white uppercase tracking-wider shadow-sm"
                    >
                      {/* THE MASKED ICON */}
                      {iconPath && (
                        <div
